...
static void rb_respond_to_sb_collision(RigidBody *rb, StaticBody *sb,
  ...
  { // impulse calculation
    ...
    float rel_mag = glm_vec3_distance(vr, (vec3){0});
    float vn = glm_vec3_dot(vr, e->direction);

    if (vn > 0.0f)
      return; // Object is moving away from static body

    if (rel_mag < 0.1) // this is a resting collision, don't bother.
      return;

    float rest_balanced =
        balance_restitutions(rb->restitution, sb->restitution);

    vec3 w_cross_r;
    glm_vec3_cross(rb->ang_velocity, r, w_cross_r);
    float inv_inertia =
        glm_vec3_dot(e->direction,
                     w_cross_r); // the resistance to the spin generated by the
                                 // external force.
  ...
  {      // friction calculation.
    do { // linear friction
      if (glm_vec3_norm(rb->velocity) < 0.001f)
        break;

      vec3 friction_dir;
      glm_vec3_normalize_to(rb->velocity, friction_dir);
      glm_vec3_negate(friction_dir);
      rb_apply_force(
          rb, friction_dir, 1,
          rb->position); // TODO: get the frictional constant, that's our mag.
    } while (0);

    do { // angular friction.
      if (glm_vec3_norm(rb->ang_velocity) < 0.01f)
        break;

      vec3 mod_velocity;
      // spin in the opposite direction to slow it down.
      glm_vec3_normalize_to(rb->ang_velocity, mod_velocity);
      rb_apply_torque(rb, mod_velocity, -1, e->contact_pt);
    } while (0);
  }
}
    ...
    if (IS_RB(body)) {
...
#ifdef DEBUG
        assert(e != NULL); // shouldn't happen with the active elements
                           // condition above.
        assert(e->from != NULL);
#endif /* ifdef DEBUG */
      ...
      w_queue_load_state(mailbox,
                         &state); // leave the data inside the queue untouched
                                  // for the object/owner of the actual
                                  // physics object to iterate through.
    }
  }
}
